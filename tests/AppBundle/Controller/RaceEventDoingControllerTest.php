...
class RaceEventDoingTest extends BaseWebTestCase
{
    public function testPostActionGreen()
    {
        ...
        $client->request('POST', $this->getRaceEventDoingUrl($raceEvent->getOid()), [], [], [
    ...
    public function testPostActionRaceEventNotFound()
    {
        ...
        $client->request('POST', $this->getRaceEventDoingUrl('invalidId'), [], [], [
            'HTTP_AUTHORIZATION' => 'Bearer '.$token,
        ]);
    ...
    public function testPostActionAlreadyExist()
    {
        ...
        $raceEvent = $raceEventRepository->findAll()[0];
        $user = $this->getUser('mattallbeury@trailburning.com');
        $raceEventDoingRepository = $this->getContainer()->get('app.repository.race_event_doing');
        $raceEventDoing = new RaceEventDoing();
        $raceEventDoing->setRaceEvent($raceEvent);
        $raceEventDoing->setUser($user);
        $raceEventDoingRepository->add($raceEventDoing);
        $raceEventDoingRepository->store();
    ...
    public function testDeleteAction()
    {
        ...
        $client->request('DELETE', $this->getRaceEventDoingUrl($raceEvent->getOid()), [], [], [
            'HTTP_AUTHORIZATION' => 'Bearer '.$token,
        ]);
    ...
    public function testDeleteActionRaceEventNotFound()
    {
        ...
        $client->request('DELETE', $this->getRaceEventDoingUrl('invalidId'), [], [], [
            'HTTP_AUTHORIZATION' => 'Bearer '.$token,
        ]);
    ...
    public function testDeleteActionRaceEventDoingNotFound()
    {
        $this->loadFixtures([
            'AppBundle\DataFixtures\ORM\UserData',
            'AppBundle\DataFixtures\ORM\RaceEventData',
    ...
    private function getRaceEventDoingUrl($raceEventId)
    {
        $url = '/v2/user/raceevents/'.$raceEventId.'/doing';
